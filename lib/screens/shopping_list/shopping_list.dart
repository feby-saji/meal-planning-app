...
class ShoppingListScreen extends StatelessWidget {
            ...
            child: BlocConsumer<ShoppingListBloc, ShoppingListState>(
              ...
              builder: (context, state) {
                if (state is ShoppingListItemsLoadedState) {
                  return _buildShoppingItems(state);
  ...
  ListView _buildShoppingItems(ShoppingListItemsLoadedState state) {
    return ListView.builder(
      padding: const EdgeInsets.only(right: 5),
      itemCount: state.categorizedItems.length,
      itemBuilder: (context, ind) {
        String itemName = state.categorizedItems.keys.elementAt(ind);
        List<ShopingListItem> innerItems =
            state.categorizedItems.values.elementAt(ind);
        double listHeight =
